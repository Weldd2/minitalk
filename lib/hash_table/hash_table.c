...
/*   Updated: 2024/01/20 19:36:45 by antoinemura      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hash_table_init(t_hashtable	*hash_table, int size, 
	...
	hash_table->hash_function = hash_function ? hash_function : &default_hash_function;
	hash_table->key_compare = key_compare ? key_compare : &default_key_compare;
}
...
{
	unsigned int	index = table->hash_function(key) % table->size;
	t_hashnode		*current = table->array[index];
	t_hashnode		*prev = NULL;
